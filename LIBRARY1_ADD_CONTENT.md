...
| `subj` | `<sys>::<slug>`. **Se o assunto não existir na lista da plataforma, usar `<sys>::misc` (Miscellaneous)** — regra explícita do usuário. |
...
**E não é só a ordem que o `inspect` pega — é a PALAVRA.** Os três defeitos da auditoria de 2026-07-26 (ver abaixo) não embaralhavam nada: trocavam letras dentro de palavras, e um PDF podia sair perfeito enquanto o vizinho saía corrompido. Ao conferir, procure também: palavra com `increased`/`decreased`/`leads to` enfiado no meio (`"st decreased uctu decreased e"`), palavra começando em vogal onde faltou `fl`/`fi` (`"uconazole"`, `"uorescent"`), e `#`/`!`/`$`/`%`/`"` sobrando no texto. A varredura rápida num PDF já extraído:

```bash
node -e "require('./tools/lib/pdf-text.js').extractPdfText(process.argv[1]).then(d=>{
  const t=d.pages.map(p=>p.text).join(' ');
  console.log('glifo sobrando:', (t.match(/[#\$%!&\"]/g)||[]).join(' '));
  console.log('palavra picada:', (t.match(/[a-z] (?:increased|decreased|leads to) [a-z]{1,3}\b/g)||[]).join(' | '));
})" .narration-build/.pdf/lib3__biochemistry__01-molecular.pdf
```

Nem todo achado é defeito: `90%`, `Stop the nonsense!` e `—Batman & Robin` são do livro, e `saturation increased in RV` é "↑ in RV" mesmo. O que não pode aparecer é glifo colado a letra minúscula.
...
2. **Glifos.** As setas do First Aid saem como letras: `q`=↑ (narrado "increased"), `r`=↓ ("decreased"), `p`=→ ("leads to"). E as ligaduras saem como pontuação — mas **o mapa varia de arquivo para arquivo**: o mesmo `!` é "ff" no PDF de psiquiatria (`Korsako!` = Korsakoff) e "fi" no de bioquímica (`el!n` = elfin). Como o PDF não diz qual é qual, a escolha é feita por palavra: primeiro um dicionário de termos médicos com ligadura, e fora dele pela fonética ("fl" só antes de vogal, "ff" entre vogais). Sem isso a narração lê "de-cifrão-ciency" e "fiuvoxamine".
   ...
   **Duas coisas aqui são pegadinha, e as duas custaram um lote inteiro de áudio (auditoria de 2026-07-26, com o lote parado antes de gerar):**

   - **O mapa de glifos só pode tocar fonte de SÍMBOLO, e "quem é fonte de símbolo" não se decide por contagem.** A primeira versão dava a fonte por símbolo quando ela produzia menos de dois fragmentos "longos" — e isso dependia de sorte. No PDF de bioquímica a fonte do CORPO vem picada em pedaços de uma letra (`"larg"`, `"l"`, `"arg"`, `"H"`), então ela caía como símbolo e todo `r` do texto normal virava "decreased": *"Chromatin structure"* era narrado **"Ch decreased omatin st decreased uctu decreased e"**, e *"Histones are large"* virava *"Histones are decreased ge"*. **Esse é o pior defeito possível neste projeto** — a frase sai plausível, com cara de informação médica, e o aluno não tem como desconfiar. Hoje a régua é o repertório: basta a fonte emitir uma letra ou dígito que uma fonte de ícone não desenha (qualquer coisa fora de `q`,`r`,`p`) para ela ser fonte de texto. E a classificação é feita numa **passada de reconhecimento sobre todas as páginas pedidas antes de extrair**, senão uma página em que a fonte do corpo só aparece num `r` solto a condenaria ali.
   - **O mesmo caractere é três coisas diferentes no mesmo PDF, e quem diz qual é a LARGURA do glifo** (em relação ao corpo da letra — medido item por item): ~1,0 em é glifo largo (travessão do título ou ligadura de três glifos, `ffi`/`ffl`), ~0,55 em é ligadura de dois glifos, ~0,28 em é espaço fino ou hífen. Fixar um papel só apagava letras sem deixar rastro: com `#` preso em "espaço" a narração dizia **"uconazole"** (fluconazole) e **"uorescent"** (fluorescent); com `!` preso em "ligadura" o cabeçalho saía **"BIOCHEMISTRYfiMOLECULAR"**; e tratar todo glifo largo como travessão fazia *Löffler endocarditis* virar **"Lö — er endocarditis"**. Entre travessão e `ffl` decide o entorno: travessão tem palavra inteira em CAIXA ALTA dos dois lados, ligadura tem minúscula em volta ou um lado curto (`E!ECTS` = EFFECTS).

   Cada um desses casos está trancado em `tools/tests/test-pdf-text.js` com os fragmentos exatos que o PDF.js entrega (fonte, string, x e largura conferidos no arquivo real). São 33 asserções; rodar `node tools/tests/test-pdf-text.js` antes de qualquer lote.
