...
## Atualização — Taxonomia UWorld completa integrada
A tela Create Test agora traz a **taxonomia oficial completa do UWorld Step 1** como accordion
de dois níveis (Sistema → Subtópico), com contadores por subtópico e por sistema, seleção
em cascata (marcar o sistema marca todos os subtópicos), colapsar/expandir e checkbox mestre
"Systems". São **26 sistemas** e **169 subtópicos** (constante `TAXONOMY` em `qbank.js`, derivada
das telas oficiais). Os subtópicos usam ids namespaced `systemId::slug`. As 10 questões-semente
foram remapeadas para ids reais da taxonomia. Assets em `?v=23`.
