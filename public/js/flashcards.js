...
(function(){
  ...
  /* ==========================================================================
     ...
     ========================================================================== */
  const FC_TAXONOMY = [
    {id:'biochemistry', name:'Biochemistry (General Principles)', subs:[['amino_acids_proteins_enzymes','Amino acids, proteins, and enzymes'],['bioenergetics_carb_metabolism','Bioenergetics and carbohydrate metabolism'],['cell_molecular_biology','Cell and molecular biology'],['lipid_metabolism','Lipid metabolism'],['misc','Others']]},
    ...
    {id:'microbiology', name:'Microbiology (General Principles)', subs:[['bacteriology','Bacteriology'],['mycology','Mycology'],['parasitology','Parasitology'],['virology','Virology'],['misc','Others']]},
    ...
    {id:'pharmacology', name:'Pharmacology (General Principles)', subs:[['drug_metabolism_toxicity','Drug metabolism and toxicity'],['drug_receptors_pharmacodynamics','Drug receptors and pharmacodynamics'],['pharmacokinetics','Pharmacokinetics'],['misc','Others']]},
  ...
  function ensureFilterState(){
    if(!view.ff) {
      ...
      FC_TAXONOMY.forEach(s => defaultCollapsed[s.id] = true);
      view.ff = { subjects:[], collapsed:defaultCollapsed, q:'', topic:'' };
    }
  ...
  function renderFilter(){
    ...
    if(sysDrop) sysDrop.addEventListener('change', e => {
      ...
      if(sysId){ ff.collapsed[sysId] = false; } // expand selected system
      renderFilter();
    });
    wire();
  }
    ...
    return `<div class="fc-rt-toolbar" data-side="${side}">
      ...
      <div class="fc-rt-popwrap">
        ...
        <div class="fc-rt-popup fc-rt-popup-up" data-popup="align_${side}" hidden>
          <button type="button" class="fc-rt-btn" data-rt="justifyLeft" data-side="${side}">≡</button>
          <button type="button" class="fc-rt-btn" data-rt="justifyCenter" data-side="${side}">≣</button>
          <button type="button" class="fc-rt-btn" data-rt="justifyRight" data-side="${side}">≢</button>
          <button type="button" class="fc-rt-btn" data-rt="justifyFull" data-side="${side}">☰</button>
        </div>
      </div>
      <button type="button" class="fc-rt-btn" data-rt="insertTable" data-side="${side}" title="${t('rtTable')}">▦</button>
      <div class="fc-rt-popwrap">
        <button type="button" class="fc-rt-btn" data-rt="listtoggle" data-side="${side}" title="${t('rtOL')}">☰̲</button>
        <div class="fc-rt-popup fc-rt-popup-up" data-popup="list_${side}" hidden>
          <button type="button" class="fc-rt-btn" data-rt="insertOrderedList" data-side="${side}">1.</button>
          <button type="button" class="fc-rt-btn" data-rt="insertUnorderedList" data-side="${side}">•</button>
        </div>
      </div>
      ...
      <div class="fc-rt-popwrap">
        ...
        <div class="fc-rt-popup fc-rt-popup-up" data-popup="img_${side}" hidden>
          <button type="button" class="fc-rt-btn" data-rt="image" data-side="${side}">📎 Upload</button>
          <button type="button" class="fc-rt-btn" data-rt="pasteimg" data-side="${side}">📋 Paste</button>
        </div>
      </div>
    </div>`;
  }
  ...
  function closeOtherPopups(side, except){
    const modal = root.querySelector('#fcModal'); if(!modal) return;
    if(except!=='color'){ const cp=root.querySelector('#fcRtColors_'+side); if(cp) cp.hidden=true; }
    if(except!=='align'){ const ap=root.querySelector('[data-popup="align_'+side+'"]'); if(ap) ap.hidden=true; }
    if(except!=='list'){ const lp=root.querySelector('[data-popup="list_'+side+'"]'); if(lp) lp.hidden=true; }
    if(except!=='img'){ const ip=root.querySelector('[data-popup="img_'+side+'"]'); if(ip) ip.hidden=true; }
  }
  function wireRichText(){
    ...
    modal.querySelectorAll('[data-rt]').forEach(el => {
      ...
      if(el.tagName==='SELECT'){
        ...
        el.addEventListener('click', e => { e.preventDefault(); const p = root.querySelector('#fcRtColors_'+side); if(p) p.hidden = !p.hidden; closeOtherPopups(side, 'color'); });
      } else if(cmd==='aligntoggle'){
        el.addEventListener('click', e => { e.preventDefault(); const p = root.querySelector('[data-popup="align_'+side+'"]'); if(p) p.hidden = !p.hidden; closeOtherPopups(side, 'align'); });
      } else if(cmd==='listtoggle'){
        el.addEventListener('click', e => { e.preventDefault(); const p = root.querySelector('[data-popup="list_'+side+'"]'); if(p) p.hidden = !p.hidden; closeOtherPopups(side, 'list'); });
      } else if(cmd==='imgtoggle'){
        el.addEventListener('click', e => { e.preventDefault(); const p = root.querySelector('[data-popup="img_'+side+'"]'); if(p) p.hidden = !p.hidden; closeOtherPopups(side, 'img'); });
