...
(function(){
  ...
  function cmFmtTime(sec,t){
    ...
    if(h>0&&m>0)return h+'<small class="unit">'+(h===1?t.hourOne:t.hourMany)+' '+t.timeAnd+' </small>'+m+'<small class="unit">'+(m===1?t.minuteOne:t.minuteMany)+'</small>';
    if(h>0)return h+'<small class="unit">'+(h===1?t.hourOne:t.hourMany)+'</small>';
    return m+'<small class="unit">'+(m===1?t.minuteOne:t.minuteMany)+'</small>';
  }
  function renderStudyTime(){
    const t=I18N[cmLang()];
    const tt=cmTimeTotals();
    const a=document.getElementById('timeToday'); if(a)a.innerHTML=cmFmtTime(tt.tToday,t);
    const b=document.getElementById('timeWeek');  if(b)b.innerHTML=cmFmtTime(tt.t7,t);
    const c=document.getElementById('timeTotal'); if(c)c.innerHTML=cmFmtTime(tt.tAll,t);
  }
