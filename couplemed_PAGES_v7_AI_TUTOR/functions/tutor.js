export async function onRequestPost(context) {
  ...
  if (!pergunta || typeof pergunta !== "string") {
    return json({ error: "Campo 'pergunta' é obrigatório" }, 400);
  }
  if (pergunta.length > 6000) {
    return json({ error: "Pergunta muito longa (máx. 6000 caracteres)" }, 400);
  }

  // Sanitização: só roles user/assistant, conteúdo string, e no máximo as últimas 16 mensagens
...
Priorize conteúdo de alto rendimento (high-yield): imunologia, cardiologia, hematologia, farmacologia, patologia.
Responda no idioma indicado pelo aluno (padrão: português), termos técnicos em inglês entre parênteses.`,

    caso: `Você conduz uma discussão de caso clínico no estilo de tutoria PBL (Problem-Based Learning).
Apresente o caso de forma progressiva (revele novos dados conforme o aluno avança no raciocínio), faça perguntas sobre hipóteses diagnósticas,
peça justificativa fisiopatológica antes de confirmar ou refutar hipóteses.
Responda no idioma indicado pelo aluno (padrão: português), com profundidade acadêmica adequada a estudante de medicina do 6º semestre.`,
...
(frente e verso separados por espaço, dois-pontos duplos, espaço; exatamente um card por linha).
